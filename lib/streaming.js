...
Streaming.prototype._createClient = function() {
  ...
  if (this._conn.proxyUrl) {
    endpointUrl = [ this._conn.proxyUrl, "cometd", this._conn.version ].join('/');
  } else {
    endpointUrl = [ this._conn.instanceUrl, "cometd", this._conn.version ].join('/');
  }
  var fayeClient = new Faye.Client(endpointUrl, {});
  var self = this;

  this._conn.on("refresh", function(token) {
    fayeClient.setHeader('Authorization', 'OAuth '+token);
    self.emit('authorize');
  });
  ...
  fayeClient.addExtension({
      incoming: function(message, callback) {
        ...
        if (message.error && message.error.match(/^401/)) {
          self.once('authorize', function() {
            //fayeClient.connect(function(){}, fayeClient);
          });
          self._conn.refresh();
        }
        callback(message);
